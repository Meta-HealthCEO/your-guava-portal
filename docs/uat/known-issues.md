# Known issues register

Carried forward so each pass starts from what is already known. Findings from the first
audit pass (Aug 2026).

## Open

| ID | Sev | Area | Issue |
| --- | --- | --- | --- |
| KI-14 | S2 | Billing | A 202 "still initializing" checkout response carries `account`, so the portal announces a plan upgrade or credit top-up that has not happened. `Account.tsx:373` |
| KI-15 | S2 | Billing | PaymentSession status `processing` is outside the portal's status union, so a settling payment is reported to the customer as failed. `Account.tsx:257` |
| KI-16 | S3 | Billing | `applyPlanPayment` re-checks plan capacity after the card is charged; failing there strands a captured payment. `billingPayments.service.js:142` |
| KI-17 | S3 | Billing | Card brand and last4 are never captured for Paystack — the adapter drops the `authorization` block that `cardDetailsFromTransaction` reads. `paystack.service.js:137` |
| KI-18 | S3 | AI | Ask Guava collapses 402 out-of-credits and 429 rate-limit into "try again in a moment", so an out-of-credits user retries indefinitely with no path to Buy Credits. Related to KI-13. `Insights.tsx:1440` |
| KI-19 | S3 | UX | Staff, Roster and Leave turn an API failure into a confident empty state, so a 500 renders as "No staff yet" with an onboarding prompt. `Staff.tsx:380` |
| KI-20 | S3 | AI | Menu AI review says "No credits were charged" on a client-side abort, but the endpoint has no disconnect handling and commits the charge. `MenuItems.tsx:269` |
| KI-21 | S3 | Uploads | `POST /transactions/upload` can reach a paid Anthropic call and a 10-credit charge with no `aiLimiter`. `transactions.routes.js:79` |
| KI-22 | S3 | Uploads | Rejected-row `raw` values are stripped on every write path, so the "Raw values" column in UploadDetail can never show anything. `uploads.controller.js:47` |
| KI-23 | S3 | Today | Dashboard ignores `availability.status === 'closed'` and renders a grid of confident zeros for a non-trading day. `Dashboard.tsx:675` |
| KI-24 | S3 | Ops | `/api/ready` is the container HEALTHCHECK target but reports storage healthy from environment-variable presence alone, never from R2 reachability. `health.controller.js:60` |
| KI-25 | S3 | Ops | SIGTERM closes the listening socket immediately with no readiness drain, so a rolling deploy refuses in-flight connections. `server.js:239` |
| KI-26 | S3 | Ops | Both background reconcilers count errors and discard the error objects, leaving nothing to diagnose from. `usage.service.js:919` |
| KI-27 | S3 | Billing | `PAYSTACK_API_URL` is read at runtime but never validated, unlike its OneGate counterpart which is pinned to the official host. `paystack.service.js:40` |
| KI-28 | S4 | Ops | Any authenticated user can flush the process-wide API cache at will via `PATCH /api/account/profile`. `account.controller.js:277` |
| KI-13 | S3 | AI | Ask Guava hangs on "Thinking…" forever when the request is refused. A manager invited without credit-spend permission gets a 403 from `/insights/chat/stream`, and the composer never resolves or shows a message. The billing page surfaces its errors correctly, so the pattern exists — the chat stream just does not use it. Found while re-testing Phase 8. |
| KI-03 | S2 | Model | Plan tier silently changes forecast output. `applyPlanEntitlements` forces locked factors off, so identical data yields different numbers by plan. Not surfaced to the user. |
| KI-05 | S3 | Uploads | Duplicate re-upload is recorded as `failed` with an `errorMessage`. The UI softens this to "no new rows" by regex-matching the message — brittle. Wants a structured reason code, which means revisiting the tested 409 contract. |
| KI-06 | S3 | Charts | Recharts still logs `width(-1)` container-measurement warnings even where charts now render. Underlying layout-timing issue unresolved. |
| KI-08 | S3 | Settings | Prediction, Integrations and Team are signpost sections. Now carry a summary line, but the IA question — inline fully, or drop from the nav — is unresolved. |
| KI-09 | S4 | Naming | `aiCredits` → `guavaCredits` rename incomplete. DB field is `org.aiCredits`; API says `guavaCredits`; plans carry both `includedAiCredits` (150) and `includedGuavaCredits` (400) with **different values**. Reading the wrong key gives a wrong number, not an error. |
| KI-10 | S4 | Ops | In-process state — API cache, multer dir, rate-limit store — prevents horizontal scaling. |

## Resolved in the first pass

| ID | Area | Was | Fix |
| --- | --- | --- | --- |
| KI-R1 | Model | Calibration averaged per-observation ratios, inflating low-volume items — Matcha (Hot) true ratio 0.787 reported as 1.035, pointing the correction the wrong way | Ratio of sums; item/factor layers residualised |
| KI-R2 | Model | Learning sample floors of 3 let payday swing demand −11.6% off three observations | Floors raised to 10 / 12 / 20 |
| KI-R3 | Model | Cafe settings did not invalidate forecasts — corrected coordinates served a stale, ~10% wrong forecast indefinitely | Location and trading-hours changes clear forecasts from today |
| KI-R4 | Charts | Revenue chart rendered nothing — animated clip rect stuck at `width="0"` over a fully-drawn path | Reveal animation disabled on all series |
| KI-R5 | Today | Variant suffixes stripped, so three different products rendered as identical cards with different numbers | Full names restored |
| KI-R6 | Today | 25 items shown with equal weight, including 24 unforecastable lines carrying <10% of volume | Confidence tiers; sub-2/day grouped as "occasional sellers" |
| KI-R7 | History | Headline accuracy was the period aggregate (97.8%) where daily errors cancel | Leads with typical day (88.1%) |
| KI-R8 | Auth | Login and signup advertised "94% Accuracy", contradicted by the product's own History page | Replaced with capability statements |
| KI-R9 | Mobile | Performance tab bar overflowed with no scroll container — "Combos" unreachable on a phone | Tab bars scroll |
| KI-R10 | Mobile | Today's KPI cards 2-across and item cards 3-across at 390px, truncating names | 1-across and 2-across below `sm` |
| KI-R11 | Analytics | Movers ranked low-volume noise — a 1 → 8 line as "+700%" | Filtered to lines averaging 2+/day, scaled to the window |
| KI-R12 | Analytics | Heatmap rendered a fixed 06:00–22:00 grid, a third permanently empty | Range derived from real trading hours |
| KI-R13 | Billing | Only the 500-credit pack was purchasable | All packs from the plan catalogue |
| KI-R14 | Menu Items | "Menu price" labelled both the current and proposed price, two lines apart | Proposal relabelled; rationale names the most recent POS price |
| KI-R15 | Integrations | Same unavailability banner repeated in all three cards | One page-level notice |
| KI-R16 (was KI-01) | Tests | Portal suite believed flaky under parallel execution; every green result used `--no-file-parallelism` | Not flakiness — a roster test called `toISOString()` on a local time, so between 00:00 and 02:00 SAST a Monday serialised as the previous Sunday. Fixed with a midday anchor and local formatting. Three consecutive full-parallelism runs green (227/227) |
| KI-R17 (was KI-02) | Auth | Signup impossible without `RESEND_API_KEY` — `VERIFICATION_EMAIL_FAILED`, stranded `pendingregistrations` row, no recovery. Team invites equally unreachable | Console transport outside production: the message is logged with its action link and reported as delivered. Token stays real, single-use, redeemed through the normal endpoint. Production still refuses; `EMAIL_DEV_CONSOLE=false` opts out |
| KI-R18 (was KI-04) | Config | Trading hours never validated against sales data — a day configured closed forecast zero despite months of sales on that weekday (cost 9pp when Sunday was mis-set) | Contradiction detection: a closed day carrying observed sales is flagged with a reason naming the transaction count and pointing at Settings |
| KI-R20 (was KI-11) | AI | Ask Guava answered relative dates with the wrong day — "what should I prepare tomorrow?" returned today's quantities under a "Tomorrow's" heading, because the context never said what today was | `currentDate` added; each forecast labelled with weekday and `relativeDay`. Weekday derived from the date key at midday UTC, since dates are stored at cafe-local midnight. Verified live: now returns Saturday 15 Aug, R4 126, Cappuccino 21 / Flat White 19 |
| KI-R21 (was KI-12) | Auth | `VerifyEmail` stripped the fragment it had just read inside the same effect, so a StrictMode remount reported "Verification failed" over a verification that returned 201 | Ref-guarded capture before paint plus a once-only request, matching `AcceptInvite`. Five tests render under StrictMode; four fail against the old implementation |
| KI-R22 | Billing | Buying credits left the toolbar showing the pre-purchase balance — 400 in the header against 900 on the page, the same number disagreeing with itself on one screen. The toolbar caches the balance for 30s and the purchase never invalidated it | Purchase publishes the new snapshot through the existing `publishGuavaCredits` channel that AI spend already used. Verified live: header moved 900 → 2 400 on purchase with no reload |
| KI-R23 | Model | Item category inference was tuned to one cafe's menu wording: 9 of 33 realistic South African menu items classified correctly. "House Coffee (Blend)" became retail because the coffee list held 'black coffee' but not 'coffee', and a bare '(Blend)' suffix was read as merchandise. Category selects the weather factor -- coffee +15% on a cold day, retail and other 0% -- so misfiled items silently lost the weather signal the product advertises | Rewritten with ordered rules, merchandise requiring an unambiguous marker, cold drinks resolved before hot, and word-boundary matching for terms hiding inside other words ('chocolate' contains 'cola'). 37/37 on the same menu, 63 unit tests |
| KI-R24 | Model | Weight normalisation accumulated floating-point error (0.35+0.25+0.20 = 0.7999...), so a true weighted average of 22.5 computed as 22.499999999999986 -- stored and displayed as 22.5 but rounded down to 22, leaving the shown base quantity and the shown prediction visibly disagreeing | Weighted average settled to 6 decimal places before rounding. Verified end to end against a control dataset: base 22.5 now predicts 23 |
| KI-R25 | Security | Unauthenticated NoSQL operator injection on the payment webhooks. The OneGate notify route took `merchant_reference` straight from the body into a Mongoose filter, and Mongoose reads an object of `$` operators as query syntax, so `{"$ne":null}` selected an arbitrary PaymentSession belonging to any tenant and the handler acted on it. Reproduced against the running server: a nonexistent string returned the app's own 404 while the operator form returned an axios gateway error, proving the query had matched | References from untrusted sources are coerced to a plain string at the webhook boundary and again inside `claimPaymentSession`, the last point before a database filter. Operator and array forms now return 400 without touching the gateway |
| KI-R26 | Billing | `PaymentSession.provider` was `enum ['mock','onegate']` while sessions are written with `providerName()`, so under `PAYMENT_PROVIDER=paystack` every checkout and credit purchase would have failed validation at creation. The pending-payment sweeper was hard-filtered to `'onegate'`, so a Paystack payment whose customer closed the tab would never have reconciled | Enum widened and a test asserts the schema accepts every provider the resolver can select; the sweeper now takes its provider list from the resolver |
| KI-R27 | Ingestion | Fractional quantities were read as their decimal digits: `0.35 x Cheese Wheel` imported as 35, `1.5 x Biltong` as 5. Cafes selling by weight had data inflated 10-100x, which flowed into forecasts, unit prices and the learning calibration | Quantity patterns accept a decimal part and the separator lookahead was widened to match. Comma decimals are deliberately unsupported, since a comma is also the item separator |
| KI-R28 | Ingestion | Receipt numbers were assumed globally unique in three places — the line-per-row grouping key, the deduplication identity, and the unique index. A till that restarts order numbers each morning had its entire import rejected as "conflicting rows" | All three scope by the cafe-local trading day. Verified: a three-day file with daily-restarting receipts imports all six sales, and re-uploading still returns 409 with the count unchanged. `src/migrations/widen-receipt-index.js` replaces the index |
| KI-R29 | Model | `updateForecastActuals` scored days that were still trading. A POS export run at midday contains today, so a partial morning was compared with a full-day forecast: accuracy 0, actualRevenue 100 against predicted 920, with `actualsUpdatedAt` set and `origin: 'live'` — precisely the filter the calibration selects on, so the model would learn it had over-predicted tenfold | Refuses to score a day that has not ended in the cafe's timezone. Completed days still score; the history route was already clamped to yesterday |
| KI-R30 | Model | Confidence was computed from predicted volume alone, so a cafe with one matching trading day of history saw "high" against an item backed by a single observation | Confidence is now the lesser of what volume justifies and what evidence supports: three or more observed weeks as before, two caps at medium, one caps at low |
| KI-R31 | Model | The three-week minimum was fixed while the Factors page lets the history lookback be set as low as one week. Observed weeks can never exceed the lookback, so the requirement was unsatisfiable and every forecast sat at `insufficient_data` for ever, blaming the data rather than the setting | The floor tracks the configured lookback and never demands more weeks than it can supply |
| KI-R32 | Model | The suggested-stock buffer measured bias only over days the item sold, discarding every negative observation, so it could only ever over-order. An item predicted at 10 selling 15 on three days and none on three suggested 17 against a true mean of 7.5 | Zero-sale days on scored trading days are counted; the same case now suggests 11. Days with no trading remain excluded |
| KI-R33 | Today | The Today page did not surface `insufficient_data`, so a new cafe saw the same numbers presented without qualification on the screen people order from. Planning already showed it | The notice now appears on Today, using the reason the API supplies |
| KI-R19 (was KI-07) | A11y | `#555555` / `#666666` muted text on near-black assumed below AA, untested | Measured: `--color-muted` was 4.49:1, now `#8A8A8A`; added `--color-guava-red-text` for red-on-dark text. Remaining `#555555` uses are non-text (backgrounds, borders, and one status dot whose meaning is carried by its adjacent label) |

## Corrected findings

Filed, then found to be wrong. Kept so they are not re-filed.

| Claim | Reality |
| --- | --- |
| "Saturdays forecast zero" | Sundays. Forecast dates are stored at local midnight, so `getUTCDay()` reports the previous day. |
| "Tables overflow on mobile with no scroll container" | A grep for `overflow-x-auto` missed `overflow-auto`, which all of them use. |
| "No tests depend on the duplicate-upload behaviour" | A test asserted the behaviour rather than the message. The change was reverted. |
| "The learning correction degrades forecasts" | The comparison toggled plan tiers, which also change payday, events and load shedding. Held constant, learning slightly helps and reduces bias. |
| "The revenue chart is slow" | It never rendered — the path was drawn and clipped to zero width. |
| "An invite sent by a second owner can never be accepted" | Acceptance requires the inviter to be `org.ownerId`, and the UAT org had two owners, so every invite 404'd. But the product never creates a second owner in an existing org: `role: 'owner'` is only assigned at registration (which creates a *new* org) or by ownership transfer (which demotes the incumbent in the same transaction). One owner per org is a real invariant. The two-owner org was a hand-inserted UAT fixture. Re-tested from an org created through the real signup flow, the whole invite journey passes. |
</content>
